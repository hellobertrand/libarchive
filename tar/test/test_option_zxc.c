...
DEFINE_TEST(test_option_zxc)
{
	...
	r = systemf("%s -cf - --zxc f >archive.out 2>archive.err",
	    testprog);
	...
	if (r != 0) {
		...
		if (strstr(p, "Can't launch") != NULL && !canZxc()) {
			skipping("This version of bsdtar uses an external zxc program "
	...
	free(p);
	/* Check that the archive file has the zxc magic word
